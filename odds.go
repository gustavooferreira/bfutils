...
// Odds is an array of decimal.Decimal holding the value of all tradable odds.
var Odds = [OddsCount]decimal.Decimal{}
...
// OddsRange is a []map[string]string with all the odd ranges available.
var OddsRange = []map[string]string{
...
var minOdd decimal.Decimal
var maxOdd decimal.Decimal

// init populates OddsDecimal with the content from oddsStr
func init() {
	for i, odd := range OddsStr {
		Odds[i], _ = decimal.NewFromString(odd)
	}

	minOdd = Odds[0]
	maxOdd = Odds[OddsCount-1]
}
...
func OddFloor(odd decimal.Decimal) (index int, oddRounded decimal.Decimal, err error) {
...
// If the odd supplied is one of the available odds in the ladder, than the same odd is returned.
// index returns the index of the odd in the ladder.
func OddCeil(odd decimal.Decimal) (index int, oddRounded decimal.Decimal, err error) {
...
// If the odd supplied is one of the available odds in the ladder, than the same odd is returned.
// If the odd supplied is at the same distance from the two closest odds it will return the lower odd.
// index returns the index of the odd in the ladder.
func OddRound(odd decimal.Decimal) (index int, oddRounded decimal.Decimal, err error) {
	match, index, err := FindOdd(odd)
	if err != nil {
		return 0, decimal.Zero, err
	}
	...
	// Compute deltas
	delta1 := odd.Sub(Odds[index]).Abs()
	delta2 := odd.Sub(Odds[index+1]).Abs()

	if delta1.LessThanOrEqual(delta2) {
		return index, Odds[index], nil
	}
	return index + 1, Odds[index+1], nil
}
...
func OddShift(roundType RoundType, odd decimal.Decimal, shift int) (index int, oddOut decimal.Decimal, err error) {
	switch roundType {
	case RoundType_Ceil:
	...
	default:
		return 0, decimal.Zero, fmt.Errorf("unrecognised round type")
	}
	...
	if (index >= OddsCount) || (index < 0) {
		return 0, decimal.Zero, fmt.Errorf("odd outside of tradable range")
	}
	return index, Odds[index], nil
}
...
// roundType is the round method to be used.
func OddsTicksDiff(roundType RoundType, odd1 decimal.Decimal, odd2 decimal.Decimal) (ticksDiff int, err error) {
	var index1 int
	var index2 int
	var err1 error
	var err2 error
	...
	ticksDiff = index2 - index1
	if ticksDiff < 0 {
		ticksDiff *= -1
	}

	return ticksDiff, nil
}

// IsOddWithinBoundaries checks if odd is within trading range.
// I.e., odd is between 1.01 and 1000.
func IsOddWithinBoundaries(odd decimal.Decimal) bool {
	if odd.GreaterThanOrEqual(minOdd) && odd.LessThanOrEqual(maxOdd) {
		return true
	}
...
func FindOdd(odd decimal.Decimal) (match bool, index int, err error) {
	// Boundaries
	if withinBoundary := IsOddWithinBoundaries(odd); !withinBoundary {
		return false, 0, fmt.Errorf("odd provided [%s] is outside of trading range", odd)
	}

	if odd.Equal(maxOdd) {
		return true, OddsCount - 1, nil
	} else if odd.Equal(minOdd) {
		return true, 0, nil
	}
	...
	for lo < hi {
		...
		if odd.Equal(Odds[mid]) {
			return true, mid, nil
		} else if odd.LessThan(Odds[mid]) {
			hi = mid
		} else {
			lo = mid + 1
		}
...
func (rt RoundType) String() string {
	return [...]string{"Round", "Floor", "Ceil"}[rt]
}
